...
Ubuntu.StyledItem {
    ...
    Item {
        ...
        width: caret ? Math.max(caret.width, units.gu(4)) : 0
        height: caret ? Math.max(caret.height, units.gu(4)) : 0
        parent: handler.main
        ...
        function moveToCaret(cx, cy) {
            ...
            draggedItem.x = cx - draggedItem.width / 2 + caret.width / 2;
            draggedItem.y = cy - draggedItem.height / 2 + caret.height / 2;
            dragger.resetDrag();
        }
