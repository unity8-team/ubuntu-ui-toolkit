...
Item {
    ...
    Item {
        ...
        //We need this because OpacityMask will draw this listview for us, but at the
        //same time we still want to get the input events!
        opacity: 0
        ...
        ListView {
            ...
            anchors.fill: parent
            anchors.leftMargin: __listViewMargin
            anchors.rightMargin: __listViewMargin
    ...
    states: [
        ...
        //one of the experiments was to only show one arrow at a time on hover
        /*State {
            name: "hoveringLeft"
            when: __hoveringLeft
            PropertyChanges { target: mask; visible: true }
            PropertyChanges { target: gradient; mirrored: true }
            PropertyChanges { target: listviewcontainer; opacity: 0.0 }
            PropertyChanges { target: hoveringIcon; visible: true; rotation: 180; anchors.leftMargin: units.gu(1) }
            AnchorChanges { target: hoveringIcon; anchors.left: hoveringArea.left; anchors.right: undefined }
        },
        State {
            name: "hoveringRight"
            when: __hoveringRight
            PropertyChanges { target: mask; visible: true }
            PropertyChanges { target: gradient; mirrored: false }
            PropertyChanges { target: listviewcontainer; opacity: 0.0 }
            PropertyChanges { target: hoveringIcon; visible: true; rotation: 0; anchors.rightMargin: units.gu(1) }
            AnchorChanges { target: hoveringIcon; anchors.right: hoveringArea.right; anchors.left: undefined }
        }*/
    ]
}
